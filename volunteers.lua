...
function displayFeed( feedName, feedURL )
	...
	if isReachable then
--THIS IS THE SOLE ORIGINAL LINE IN THE ORIGINAL CODE		network.download( feedURL, "GET", networkListener, feedName, system.CachesDirectory )
--EVERYTHING BELOW HERE IS BULLS***
		local lolpath = system.pathForFile( feedName, system.ResourcesDirectory )
		local lolfile, lolerrorString = io.open( lolpath, "r" )
		local lolxml = lolfile:read( "*a" )
		local path = system.pathForFile( feedName, system.CachesDirectory )
		local file, errorString = io.open( path, "w" )
		file:write( lolxml )
		io.close( file )
		processRSSFeed( feedName, system.CachesDirectory )
--EVERYTHING ABOVE HERE IS BULL C***
	else
		local path = system.pathForFile( feedName, system.CachesDirectory )
