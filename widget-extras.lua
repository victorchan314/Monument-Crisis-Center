...
function widget.newSharingPanel( services )
    ...
    local function onRowRender( event )
        ...
        if row.isCategory then
            row.text = display.newText("Share", 0, 0, mccapp.font, 14)
            row.text:setFillColor( 0.67 )
            row.text.x = display.contentCenterX
            row.text.y = row.contentHeight * 0.5
            row:insert(row.text)
        else
            row.text = display.newText(params.label, 0, 0, mccapp.font, 18)
...
function widget.newNavigationBar( options )
    ...
    if opt.background then
        barContainer._background = display.newImage(barContainer, opt.background, background.x, background.y)
	barContainer._background.width = background.width
	barContainer._background.height = background.height
    ...
    opt.titleX = customOptions.titleX or background.x
    opt.titleY = customOptions.titleY or background.y + statusBarPad * 0.5
    opt.titleLeft = customOptions.titleLeft or nil

    if opt.titleLeft then
        barContainer._title = display.newText(opt.title, opt.titleLeft, opt.titleY, opt.font, opt.fontSize)
        barContainer._title.anchorX = 0
    else
        barContainer._title = display.newText(opt.title, opt.titleX, opt.titleY, opt.font, opt.fontSize)
    end
